...
export async function deleteReservation(reservationId) {
  ...
  const userReservationByEmail = userReservations.map(
    (reservation) => reservation.guestEmail
  );

  if (!userReservationByEmail.includes(session.user.email))
    throw new Error(" You can only delete your reservation");
...
export async function createMessage(formData) {
  ...
  const { error } = await supabase.from("contactMessages").insert([newMessage]);
